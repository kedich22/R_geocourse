...
ggplot(filter_data, mapping = aes(Datetime, `T`, color = NAME)) +
  geom_point() +
  ggtitle('Плотности распределения температур\n на станциях "Малое Сараево", "Балчуг"') +
  ylab('C°') +
  xlab('Месяц') +
...
ggplot(filter_data_night, mapping = aes(Datetime, `T`, color = NAME)) +
  geom_point() +
  ggtitle('Плотности распределения ночных температур\nна станциях "Малое Сараево", "Балчуг"') +
  ylab('C°') +
  xlab('Месяц') +
  labs(color = 'Метеостанция')
```
...
На основе полученных данных можно сказать что величины имеют незначительную обратную связь, так как значение всего -0.41.
...
Выявлена незначительная обратная связь. Можно отметить, что в ночное время связь менее значительна (-0.33).
